...
import {
  ...
  pickFileUploadRecordFields,
} from "./FileTransfer/utils";
...
export class ApiClient extends ModuleApiBase {
  ...
  constructor(app) {
    ...
    this.commands = {
      ...this.commands,
      message_send: {
        method: this.message_send.bind(this),
        reqUserSession: true,
      },
      message_seen: {
        method: this.message_seen.bind(this),
        reqUserSession: true,
      },
      message_delete: {
        method: this.message_delete.bind(this),
        reqUserSession: true,
      },
      message_reaction: {
        method: this.message_reaction.bind(this),
        reqUserSession: true,
      },
      messages_list: {
        method: this.messages_list.bind(this),
        reqUserSession: true,
      },
      conversations_list: {
        method: this.conversations_list.bind(this),
        reqUserSession: true,
      },
      upload_begin: {
        method: this.upload_begin.bind(this),
        reqUserSession: true,
      },
      upload_chunk: {
        method: this.upload_chunk.bind(this),
        reqUserSession: true,
      },
      ...
      download_chunk: {
        method: this.download_chunk.bind(this),
        reqUserSession: true,
      },
      upload_update_status: {
        method: this.upload_update_status.bind(this),
        reqUserSession: true,
      },
    };
  ...
  async runFileUploadsCleanup() {
    while (true) {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, FILE_TRANSFER_SETTINGS.CLEANUP_INTERVAL_MS),
      );
      ...
      const inMemoryRecords = Array.from(
        this.app.fileTransferManager.records.values(),
      );
      // combine both where memory records have priority
      const records = [...inMemoryRecords, ...dbRecords].reduce(
        (acc, record) => {
      ...
      await this.app.fileTransferManager.checkAndValidateFileUploads(
        dbRecords,
        async (updatedRecord: FileUploadRecord) => {
  ...
  async download_chunk(c) {
    ...
    if (record.type === FileUploadRecordType.SERVER) {
      const { chunk } = await this.app.fileTransferManager.getFileChunk(
        uploadId,
        offsetBytes,
        chunkSize,
      );
      ...
      try {
        ...
        const { chunk } = await this.app.fileTransferManager.getFileChunkP2P(
          uploadId,
          chunkId,
        );
        // prefetch
        // todo: make better & support dynamic chunksize in future
        const existingP2PChunks =
          this.app.fileTransferManager.p2pTempChunks.get(uploadId);
        ...
        if (e instanceof DownloadChunkP2PNotFoundError) {
          ...
          return {
            ...
            message: "Chunk could not be obtained",
          };
        }
      }
    } else {
  ...
  async upload_get(c) {
    ...
    try {
      ...
      const owners = await this.app.data.getAttachmentsByFileTransferId(
        record.id,
      );
      ...
      return {
        error: false,
        data: {
          record: pickFileUploadRecordFields(
            record,
            UPLOAD_RECORD_PICKED_FIELDS_FOR_FRONTEND,
          ),
          uploadData: {
            uploadedBytes: record.chunksReceived.length * record.chunkSize,
            role:
              c.userID === record.fromUserId
                ? FileUploadRole.SENDER
                : FileUploadRole.RECEIVER,
          },
        },
      };
    } catch (err) {
  ...
  async upload_begin(c) {
    ...
    if (!records)
      return { error: "RECORDS_MISSING", message: "Records are missing" };
    if (!recipients)
      return { error: "RECIPIENTS_MISSING", message: "Recipients are missing" };
    ...
    for (let record of records) {
      ...
      await this.app.data.createAttachment(
        makeAttachmentRecord({
          ...
          filePath:
            updatedRecord.type === FileUploadRecordType.P2P
              ? null
              : FILE_TRANSFER_SETTINGS.SERVER_TRANSFER.USER_FILE_NAME_STRATEGY(
                  updatedRecord,
                ),
        }),
      );
      ...
      for (let recipientAddress of recipients) {
        ...
        const userToID = await this.core.api.getUserIDByUsernameAndDomainID(
          usernameTo,
          domainToID,
        );
        await this.app.data.createAttachment(
  ...
  async upload_update_status(c) {
    ...
    if (!record)
      return { error: "RECORD_NOT_FOUND", message: "Record not found" };
    const updateStatusAndSendNotification = async (
    ...
    if (newStatus === FileUploadRecordStatus.CANCELED) {
      ...
      if (record.status !== FileUploadRecordStatus.UPLOADING) {
        return {
          error: "INVALID_STATUS_CHANGE",
          message: "Invalid status change to PAUSED from " + record.status,
        };
      }
      ...
      if (record.status !== FileUploadRecordStatus.PAUSED) {
        return {
          error: "INVALID_STATUS_CHANGE",
          message: "Invalid status change to UPLOADING from " + record.status,
        };
      }
  ...
  async send_upload_update_notification(
    { record, uploadData }: { record: FileUploadRecord; uploadData?: any },
    ignoreUserIds = [],
  ) {
    return await this.app.data
      .getAttachmentsByFileTransferId(record.id)
      .then((attachments) => {
  ...
  async message_send(c) {
    if (!c.params)
      return { error: "PARAMETERS_MISSING", message: "Parameters are missing" };
    if (!c.params.address)
      return {
    ...
    if (!domainToID)
      return {
        ...
        message: "Domain name not found on this server",
      };
    const userToID = await this.core.api.getUserIDByUsernameAndDomainID(
      usernameTo,
      domainToID,
    ...
    if (!userToID)
      return {
        ...
        message: "User name not found on this server",
      };
    const userFromInfo = await this.core.api.userGetUserInfo(c.userID);
    const userFromDomain = await this.core.api.getDomainNameByID(
      userFromInfo.id_domains,
    );
    const userFromAddress = userFromInfo.username + "@" + userFromDomain;
    if (!c.params.message)
      return { error: "MESSAGE_MISSING", message: "Message is missing" };
    let format = c.params.format ? c.params.format : "plaintext";
    if (!this.isEnumValue(format))
      return {
    ...
    if (!c.params.uid)
      return {
        error: "MESSAGE_UID_MISSING",
        message: "Message UID is missing",
      };
    ...
    const msg1_insert = await this.app.data.createMessage(
      c.corr,
      c.userID,
      uid,
      userFromAddress,
    ...
    if (userToID !== userFromInfo.id) {
      ...
      const msg2_insert = await this.app.data.createMessage(
        c.corr,
        userToID,
        uid,
        userToAddress,
  ...
  async message_reaction(c) {
    if (!c.params)
      return { error: "PARAMETERS_MISSING", message: "Parameters are missing" };
    if (!c.params.messageUid)
      return {
        error: "MESSAGE_UID_MISSING",
        message: "Message UID is missing",
      };
    if (!c.params.operation)
      return { error: "OPERATION_MISSING", message: "Operation is missing" };
    if (!c.params.reaction)
      return {
        error: "REACTION_OBJECT_MISSING",
        message: "Reaction object is missing",
      };
    if (!c.params.reaction.emoji_codepoints_rgi)
      return {
        error: "REACTION_NOT_SPECIFIED",
        message: "You must define emoji_codepoints_rgi in the reaction object",
      };
    ...
    const dispatchNotification = async () => {
      const recipients =
        await this.app.repos.messages.findMessageRecipients(messageUid);
      const currentReactions =
        await this.app.repos.messagesReactions.getSingleMessageReactions(
          messageUid,
        );
    ...
    if (operation === "set") {
      const res =
        await this.app.services.messagesReactions.setUserMessageReaction(
          userId,
          userFromAddress,
          messageUid,
          reaction,
      ...
      const res =
        await this.app.services.messagesReactions.unsetUserMessageReaction(
          userId,
          userFromAddress,
          messageUid,
          reaction,
  ...
  async message_seen(c) {
    ...
    let result = await this.message_seen_mutex.runExclusive(async () => {
      ...
      if (!res)
        return { error: "WRONG_MESSAGE_ID", message: "Wrong message ID" };
      //Log.debug(c.corr, 'res....seen:', res);
      if (res.seen)
        return {
          error: "SEEN_ALREADY_SET",
          message: "Seen flag was already set",
        };
    ...
    const userFromID = await this.core.api.getUserIDByUsernameAndDomainName(
      username,
      domain,
    );
  ...
  async messages_list(c) {
    ...
    const messages = await this.app.data.userListMessages(
      c.userID,
      c.userAddress,
      c.params.address,
      c.params?.base,
  ...
  async conversations_list(c) {
    const conversations = await this.app.data.userListConversations(
      c.userID,
      c.userAddress,
    );
